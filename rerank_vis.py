...
@st.cache_data
def load_refcoco():
    return load_dataset("lmms-lab/RefCOCO", split="val")

refcoco = load_refcoco()

def get_image_by_filename(filename):
    for ex in refcoco:
        if ex["image"].filename.endswith(filename):
            return ex["image"]
    return None
...
if image is None:
    st.error(f"Image {filename} not found in RefCOCO.")
    st.stop()
...
def reranked_table(candidates, distances, title, key):
...
with col1:
    reranked_table(candidates, gaze_scores, "👁️ Gaze-Based Ranking", "gaze")

with col2:
    reranked_table(candidates, rec_scores, "📍REC-Based Ranking", "rec")
